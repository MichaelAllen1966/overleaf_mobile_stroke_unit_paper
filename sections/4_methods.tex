...
* For patients first attending a CSC in the usual care pathway, note that \emph{hospital arrival to IVT} and \emph{hospital arrival to MT} are overlapping durations (see figure \ref{fig:process}).

In the usual care pathway, the door-to-puncture time is likely to be shorter for those patients living closest to a PSC and requiring an onwards transfer to a CSC for MT, than for those patients who are directly admitted to the CSC. This is due to some of the necessary processes (such as checks and scans) already being completed at the PSC. We will refer to this duration (from arrival at PSC to delivery of MT at the CSC) as the \textit{net delay in MT}. It is represented in the model by the choice of values for \emph{hospital arrival to IVT} + \emph{transfer-related delay} + \textit{hospital arrival to MT}, plus transfer travel time. For example, if the time spent in the PSC is 90 minutes (known as the \textit{door-in-door-out} time, represented in the model as \emph{hospital arrival to IVT} + \emph{transfer-related delay}), but the \textit{hospital arrival to MT} is reduced by 30 minutes at the CSC (represented in the model as changing the \emph{hospital arrival to MT} from 90 to 60, say) due to some of the processes already being completed at the PSC, the duration of 60 minutes + transfer travel time will be referred to as the \textit{net delay in MT}.

\textbf{IF YOU ARE HAPPY TO WORK ON THE ABOVE PARAGRAPH, DELETE THIS PARAGRAPH:}
For the usual care pathway, when onward transfer to a CSC for MT is required (due to the patient living closest to a PSC), time to MT at the CSC is often likely to be shorter than time to MT for patients directly admitted to the CSC. This is accounted for in the \textit{transfer-related delay} parameter; for example if the time spent in the PSC is 90 minutes (known as the \textit{door-in-door-out} time), but the \textit{hospital arrival to MT} is reduced by 30 minutes at the CSC, this is represented as a \textit{net delay} in MT of 60 minutes + transfer travel time, and is constructed by the scenario with these values: \emph{hospital arrival to IVT}: 30, \emph{transfer-related delay}: 30.
